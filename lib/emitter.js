...
function Emitter(baseModule) {
	...
	if (!this.hub) {
		...
		var ensureHub = function () {
			process.removeListener && process.removeListener('exit', ensureHub);
		...
		process.on && process.on('exit', ensureHub);	// In case the process exits during the first tick, deliver logs anyway
	}
