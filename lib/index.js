...
exports.defaults = require('./level-set');
...
function ensurePackage(info) {
	...
	// Don't die if browserify fails to resolve our package from absolute path (before logger exists)
	if (!info.body) {
		logger && logger.warn("Couldn't locate package.json for " + info.dir + ".  Falling back to dummy package info. Are you running in Browserify without browserify-logup-packages?");
		info.body = {};
	} else
		logger && logger.warn("require('" + info.dir + "/package.json') doesn't have package name.  Falling back to dummy package info. Are you running in Browserify without browserify-logup-packages?");
