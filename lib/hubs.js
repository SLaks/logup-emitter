...
if (process.browser) {
	...
	StubHub.prototype.log = function (source, level, timestamp, message, data) {
		...
		if (arguments.length > 5)
			...
			data = "";
		console[consoleMethods[level] || consoleMethods.default](level, new Date(timestamp).toISOString(), message, data);
	};
} else {
	StubHub.prototype.log = function (source, level, timestamp, message, data) {
		if (levels.values[level] < this.minLevel)
		...
		if (arguments.length > 4)
			...
			data = util.inspect(data, { colors: true });
		// TODO: Colorize prefixes
		console.error(level, new Date(timestamp).toISOString(), message, data);
	};
}