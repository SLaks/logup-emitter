...
var defaultLevels = require('./env-defaults');
...
StubHub.prototype.checkLevel = function (source, e) {
	e.respond(defaultLevels.env.forPackage(source.packageInfo.name));
};
...
if (process.browser) {
	...
	StubHub.prototype.log = function (source, level, timestamp, message, data) {
		if (levels.values[level] < defaultLevels.env.forPackage(source.packageInfo.name))
			return;
